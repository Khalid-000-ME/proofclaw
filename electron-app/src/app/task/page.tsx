...
export default function TaskPage() {
  ...
  useEffect(() => {
    fetch(getApiUrl('/api/config/contracts'))
      .then((r) => r.json())
      .then((cfg) => {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const provRes = await fetch(getApiUrl(`/api/tasks/providers?taskId=${encodeURIComponent(activeTaskId)}`))
  ...
  useEffect(() => {
    ...
    const fetchRes = async () => {
      try {
        const res = await fetch(getApiUrl(`/api/tasks/result?taskId=${encodeURIComponent(activeTaskId)}`))
